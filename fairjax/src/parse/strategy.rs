            ...
            1 => {
                if let Meta::Path(strategy_ident) = &input[0].meta {
                    match strategy_ident.get_ident() {
                        Some(ident) if ident == "Auto" => Ok(Strategy::Auto),
                        Some(ident) if ident == "StatefulTree" => Ok(Strategy::StatefulTree),
                        Some(ident) if ident == "BruteForce" => Ok(Strategy::BruteForce),
...
mod tests {
    ...
    fn test_parse_empty_vec_returns_auto() {
        ...
        let result = Strategy::parse(attrs).unwrap();
    ...
    fn test_parse_stateful_tree() {
        ...
        let result = Strategy::parse(attrs).unwrap();
        assert_eq!(result, Strategy::StatefulTree);
    }
